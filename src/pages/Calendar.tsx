...
export default function Calendar() {
  ...
  /**
   * When a time block form is submitted, either update an existing block or
   * create a new one.  We automatically close the modal afterwards.
   */
                  ...
                  {timeBlocks.map(tb => {
                    ...
                    return (
                      <div key={tb.id} className="flex items-center justify-between p-2 rounded-lg bg-muted">
                        <span>
                          {category ? category.label : "Uncategorised"} : {tb.startTime} - {tb.endTime}
                        </span>
                        <div>
                          <Button variant="ghost" size="sm" onClick={() => {
